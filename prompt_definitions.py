...
**当前章节摘要生成规则**：
1. 内容架构：
   - 继承权重：60%内容需与前3章形成逻辑递进
   - 爽点密度：每300-500字必须包含1个“爽点”（如主角逆袭、打脸反派、获得金手指）
   - 创新空间：40%内容可引入新冲突，优先选择“系统提示”“能力升级”“隐藏剧情解锁”等类型
2. 结构控制：
   - 采用"承继→发展→铺垫"三段式结构
   - 每段含1个前文呼应点+1个新进展
3. 预警机制：
...
请设计1个具有动态变化潜力的核心角色，该角色需包含：
特征：
- 背景、外貌、性别、年龄、职业等
- 暗藏的秘密或潜在弱点(可与世界观或其他角色有关)

核心驱动力三角：
- 表面追求（物质目标，需与金手指直接关联，如“通过系统任务获取财富”）
- 深层渴望（情感需求，如“证明被轻视的自己”）
- 灵魂需求（哲学层面，如“打破阶级固化的规则”）
- 金手指设定：[具体能力] + [限制条件] + [升级路径]（如“读心术，但每天限用3次，完成任务可解锁次数”）
...
伏笔操作：埋设(A线索)→强化(B矛盾)...
认知颠覆：★☆☆☆☆
本章简述：[一句话概括，需包含1个未解决的冲突或未知结果]
结尾钩子：[必须包含以下类型之一：
  - 突发危机（如“主角刚获得能力就被反派围堵”）
  - 信息差（如“他不知道，身边的盟友正是幕后黑手”）
  - 系统提示（如“叮！隐藏任务触发，失败将抹除存在”）
]
