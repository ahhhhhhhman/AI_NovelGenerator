...
def build_chapters_tab(self):
    ...
    prev_btn = ctk.CTkButton(top_frame, text=_("<< 上一章"), command=self.prev_chapter, font=("Microsoft YaHei", 12))
    prev_btn.grid(row=0, column=0, padx=5, pady=5, sticky="w")

    next_btn = ctk.CTkButton(top_frame, text=_("下一章 >>"), command=self.next_chapter, font=("Microsoft YaHei", 12))
    ...
    save_btn = ctk.CTkButton(top_frame, text=_("保存修改"), command=self.save_current_chapter, font=("Microsoft YaHei", 12))
    save_btn.grid(row=0, column=3, padx=5, pady=5, sticky="w")

    refresh_btn = ctk.CTkButton(top_frame, text=_("刷新章节列表"), command=self.refresh_chapters_list, font=("Microsoft YaHei", 12))
    refresh_btn.grid(row=0, column=5, padx=5, pady=5, sticky="e")

    self.chapters_word_count_label = ctk.CTkLabel(top_frame, text=_("字数：0"), font=("Microsoft YaHei", 12))
    ...
    def update_word_count(event=None):
        ...
        self.chapters_word_count_label.configure(text=_(f"字数：{text_length}"))
...
def refresh_chapters_list(self):
    ...
    if not os.path.exists(chapters_dir):
        self.safe_log(_("尚未找到 chapters 文件夹，请先生成章节或检查保存路径。"))
...
def load_chapter_content(self, chapter_number_str):
    ...
    if not os.path.exists(chapter_file):
        self.safe_log(_(f"章节文件 {chapter_file} 不存在！"))
        return
...
def save_current_chapter(self):
    ...
    if not chapter_number_str:
        messagebox.showwarning(_("警告"), _("尚未选择章节，无法保存。"))
        return
    filepath = self.filepath_var.get().strip()
    if not filepath:
    ...
    self.safe_log(_(f"已保存对第 {chapter_number_str} 章的修改。"))
...
def prev_chapter(self):
    ...
    if idx > 0:
        ...
        messagebox.showinfo(_("提示"), _("已经是第一章了。"))
...
def next_chapter(self):
    ...
    if idx < len(self.chapters_list) - 1:
        ...
        messagebox.showinfo(_("提示"), _("已经是最后一章了。"))
