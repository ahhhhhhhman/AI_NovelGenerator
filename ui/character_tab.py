...
def build_character_tab(self):
    ...
    load_btn = ctk.CTkButton(self.character_tab, text=_("加载 character_state.txt"), command=self.load_character_state, font=("Microsoft YaHei", 12))
    load_btn.grid(row=0, column=0, padx=5, pady=5, sticky="w")

    self.character_wordcount_label = ctk.CTkLabel(self.character_tab, text=_("字数：0"), font=("Microsoft YaHei", 12))
    self.character_wordcount_label.grid(row=0, column=1, padx=5, pady=5, sticky="w")

    save_btn = ctk.CTkButton(self.character_tab, text=_("保存修改"), command=self.save_character_state, font=("Microsoft YaHei", 12))
    ...
    def update_word_count(event=None):
        ...
        self.character_wordcount_label.configure(text=_("字数：{0}").format(text_length))
...
def load_character_state(self):
    ...
    self.log(_("已加载 character_state.txt 到编辑区。"))
...
def save_character_state(self):
    ...
    self.log(_("已保存对 character_state.txt 的修改。"))
