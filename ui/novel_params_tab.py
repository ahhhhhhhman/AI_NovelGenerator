...
def build_novel_params_area(self, start_row=1):
    ...
    # 1) 主题(Topic)
    create_label_with_help_for_novel_params(self, parent=self.params_frame, label_text=_("主题(Topic):"), tooltip_key="topic", row=0, column=0, font=("Microsoft YaHei", 12), sticky="ne")
    ...
    # 2) 类型(Genre)
    create_label_with_help_for_novel_params(self, parent=self.params_frame, label_text=_("类型(Genre):"), tooltip_key="genre", row=1, column=0, font=("Microsoft YaHei", 12))
    ...
    create_label_with_help_for_novel_params(self, parent=self.params_frame, label_text=_("章节数 & 每章字数:"), tooltip_key="num_chapters", row=row_for_chapter_and_word, column=0, font=("Microsoft YaHei", 12))
    ...
    num_chapters_label = ctk.CTkLabel(chapter_word_frame, text=_("章节数:"), font=("Microsoft YaHei", 12))
    ...
    word_number_label = ctk.CTkLabel(chapter_word_frame, text=_("每章字数:"), font=("Microsoft YaHei", 12))
    ...
    # 4) 保存路径
    row_fp = 3
    create_label_with_help_for_novel_params(self, parent=self.params_frame, label_text=_("保存路径:"), tooltip_key="filepath", row=row_fp, column=0, font=("Microsoft YaHei", 12))
    ...
    browse_btn = ctk.CTkButton(self.filepath_frame, text=_("浏览..."), command=self.browse_folder, width=60, font=("Microsoft YaHei", 12))
    browse_btn.grid(row=0, column=1, padx=5, pady=5, sticky="e")

    # 5) 章节号
    row_chap_num = 4
    create_label_with_help_for_novel_params(self, parent=self.params_frame, label_text=_("章节号:"), tooltip_key="chapter_num", row=row_chap_num, column=0, font=("Microsoft YaHei", 12))
    ...
    # 6) 内容指导
    row_user_guide = 5
    create_label_with_help_for_novel_params(self, parent=self.params_frame, label_text=_("内容指导:"), tooltip_key="user_guidance", row=row_user_guide, column=0, font=("Microsoft YaHei", 12), sticky="ne")
    ...
    row_idx = 6
    create_label_with_help_for_novel_params(self, parent=self.params_frame, label_text=_("核心人物:"), tooltip_key="characters_involved", row=row_idx, column=0, font=("Microsoft YaHei", 12))
    ...
    # 导入按钮
    import_btn = ctk.CTkButton(char_inv_frame, text=_("导入"), width=60, 
                             command=self.show_character_import_window,
                             font=("Microsoft YaHei", 12))
    import_btn.grid(row=0, column=1, padx=(0,5), pady=5, sticky="e")
    row_idx += 1
    create_label_with_help_for_novel_params(self, parent=self.params_frame, label_text=_("关键道具:"), tooltip_key="key_items", row=row_idx, column=0, font=("Microsoft YaHei", 12))
    ...
    row_idx += 1
    create_label_with_help_for_novel_params(self, parent=self.params_frame, label_text=_("空间坐标:"), tooltip_key="scene_location", row=row_idx, column=0, font=("Microsoft YaHei", 12))
    ...
    row_idx += 1
    create_label_with_help_for_novel_params(self, parent=self.params_frame, label_text=_("时间压力:"), tooltip_key="time_constraint", row=row_idx, column=0, font=("Microsoft YaHei", 12))
...
def build_optional_buttons_area(self, start_row=2):
    ...
    self.btn_check_consistency = ctk.CTkButton(
        self.optional_btn_frame, text=_("一致性审校"), command=self.do_consistency_check, 
    ...
    self.btn_import_knowledge = ctk.CTkButton(
        self.optional_btn_frame, text=_("导入知识库"), command=self.import_knowledge_handler,
    ...
    self.btn_clear_vectorstore = ctk.CTkButton(
        self.optional_btn_frame, text=_("清空向量库"), fg_color="red", 
    ...
    self.plot_arcs_btn = ctk.CTkButton(
        self.optional_btn_frame, text=_("查看剧情要点"), command=self.show_plot_arcs_ui,
        font=("Microsoft YaHei", 12), width=100
    )
    ...
    self.role_library_btn = ctk.CTkButton(
        self.optional_btn_frame, text=_("角色库"), command=self.show_role_library,
        font=("Microsoft YaHei", 12), width=100
    )
...
def create_label_with_help_for_novel_params(self, parent, label_text, tooltip_key, row, column, font=None, sticky="e", padx=5, pady=5):
    ...
    btn = ctk.CTkButton(frame, text=_("?"), width=22, height=22, font=("Microsoft YaHei", 10),
