...
def build_setting_tab(self):
    ...
    load_btn = ctk.CTkButton(self.setting_tab, text=_("加载 Novel_architecture.txt"), command=self.load_novel_architecture, font=("Microsoft YaHei", 12))
    load_btn.grid(row=0, column=0, padx=5, pady=5, sticky="w")

    self.setting_word_count_label = ctk.CTkLabel(self.setting_tab, text=_("字数：0"), font=("Microsoft YaHei", 12))
    self.setting_word_count_label.grid(row=0, column=1, padx=5, pady=5, sticky="w")

    save_btn = ctk.CTkButton(self.setting_tab, text=_("保存修改"), command=self.save_novel_architecture, font=("Microsoft YaHei", 12))
    ...
    def update_word_count(event=None):
        ...
        count = len(text) - 1
        self.setting_word_count_label.configure(text=_("字数：{0}").format(count))
...
def load_novel_architecture(self):
    ...
    if not filepath:
        messagebox.showwarning(_("警告"), _("请先设置保存文件路径"))
        return
    ...
    self.log(_("已加载 Novel_architecture.txt 内容到编辑区。"))
...
def save_novel_architecture(self):
    ...
    self.log(_("已保存对 Novel_architecture.txt 的修改。"))
