...
def build_summary_tab(self):
    ...
    load_btn = ctk.CTkButton(self.summary_tab, text=_("加载 global_summary.txt"), command=self.load_global_summary, font=("Microsoft YaHei", 12))
    load_btn.grid(row=0, column=0, padx=5, pady=5, sticky="w")

    self.word_count_label = ctk.CTkLabel(self.summary_tab, text=_("字数：0"), font=("Microsoft YaHei", 12))
    self.word_count_label.grid(row=0, column=1, padx=5, pady=5, sticky="w")

    save_btn = ctk.CTkButton(self.summary_tab, text=_("保存修改"), command=self.save_global_summary, font=("Microsoft YaHei", 12))
    ...
    def update_word_count(event=None):
        ...
        count = len(text) - 1
        self.word_count_label.configure(text=_("字数：{0}").format(count))
...
def load_global_summary(self):
    ...
    self.log(_("已加载 global_summary.txt 到编辑区。"))
...
def save_global_summary(self):
    ...
    self.log(_("已保存对 global_summary.txt 的修改。"))
