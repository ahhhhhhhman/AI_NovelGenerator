...
def build_directory_tab(self):
    ...
    load_btn = ctk.CTkButton(self.directory_tab, text=_("加载 Novel_directory.txt"), command=self.load_chapter_blueprint, font=("Microsoft YaHei", 12))
    load_btn.grid(row=0, column=0, padx=5, pady=5, sticky="w")

    self.directory_word_count_label = ctk.CTkLabel(self.directory_tab, text=_("字数：0"), font=("Microsoft YaHei", 12))
    self.directory_word_count_label.grid(row=0, column=1, padx=5, pady=5, sticky="w")

    save_btn = ctk.CTkButton(self.directory_tab, text=_("保存修改"), command=self.save_chapter_blueprint, font=("Microsoft YaHei", 12))
    ...
    def update_word_count(event=None):
        ...
        count = len(text) - 1
        self.directory_word_count_label.configure(text=_("字数：{0}").format(count))
...
def load_chapter_blueprint(self):
    ...
    self.log(_("已加载 Novel_directory.txt 内容到编辑区。"))
...
def save_chapter_blueprint(self):
    ...
    self.log(_("已保存对 Novel_directory.txt 的修改。"))
